...
let particles = [];
const particleCount = 55; // 50 to 60 particles

const floralSymbols = ["✿", "❀", "🌸", "🌺", "🌷", "🌿", "🍃"];
// Custom hex color palette specified by user
const particleColors = ["#f2c4b0", "#c9736a", "#6b8f5e", "#f7e8e0"];
...
class BloomParticle {
    ...
    reset(isInitial) {
        ...
        // Pure upward Y velocity (0.4 to 1.5) - no gravity
        this.speed = Math.random() * (1.5 - 0.4) + 0.4;
        
        // Sinusoidal X drift variables
        this.oscillate = Math.random() * Math.PI * 2;
        this.oscillateSpeed = Math.random() * 0.02 + 0.01; // gentle frequency
        
        // Rotation and scale
        this.angle = Math.random() * Math.PI * 2;
        this.rotSpeed = (Math.random() - 0.5) * 0.02; // random rotation speed
        this.scale = Math.random() * 0.6 + 0.5; // varied scale sizing
        
        // Soft opacity (0.1 to 0.5) for dreamlike depth
        this.alpha = Math.random() * 0.4 + 0.1;
        
        // Select custom color from requested list
        this.color = particleColors[Math.floor(Math.random() * particleColors.length)];
        
        // Mix of emojis and soft ellipse shapes
        this.isTextSymbol = Math.random() > 0.5;
        if (this.isTextSymbol) {
            ...
            this.ellipseRx = Math.random() * 6 + 6; // semi-major radius
            this.ellipseRy = this.ellipseRx * (Math.random() * 0.4 + 1.2); // semi-minor radius
        }
    }
    ...
    update() {
        ...
        // Sinusoidal X drift using oscillate += oscillateSpeed then x += sin(oscillate) * 0.8
        this.oscillate += this.oscillateSpeed;
        this.x += Math.sin(this.oscillate) * 0.8;
        
        // Rotation
        this.angle += this.rotSpeed;

        // Reset condition: On reaching y < -50, reset to y = canvas.height + 30 at random X
        if (this.y < -50) {
            this.reset(false);
        }
    }
    ...
    draw() {
        ...
        // Apply soft alpha transparency (0.1 - 0.5)
